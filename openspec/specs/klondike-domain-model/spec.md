# klondike-domain-model Specification

## Purpose

Defines the Klondike Solitaire domain model — the card/pile data structures using standard terminology, standard-deck construction, deterministic seeded deal/shuffle, game configuration (draw mode, redeal limit), and the `GameState` aggregate that is the single source of truth for a game in progress.
