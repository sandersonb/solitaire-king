...
Defines how the CLI interprets player input for Klondike: the pile key mapping, source-then-target move construction, stock dealing and recycling, the waste as a move source, Enter auto-assign, forgiving foundation targeting, tableau run-length selection, and rejection of illegal moves.
