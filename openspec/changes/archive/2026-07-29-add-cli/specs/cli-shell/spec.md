...
- **WHEN** the binary is run with an unsupported value such as `--draw 2`
...
The CLI SHALL let the player start a fresh game (a new random seed) with `n`, quit with `q`, and SHALL, on a win, display the final score — including the timed bonus when timed mode is enabled — and the elapsed time.
