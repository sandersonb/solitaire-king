...
**Reason**: Selection highlighting is removed with the move to drag-and-drop; this requirement's concerns are replaced by "Status, seed, and drag feedback display", which indicates the dragged card instead of a persistent selection and shows the seed as a pronounceable string.
**Migration**: The active card is now shown by drawing it under the pointer while dragging rather than by a highlight on a selected pile; seed/move/score/time and rejected-move feedback are otherwise unchanged.
